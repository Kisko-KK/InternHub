...
import { Internship, Paging, InternshipFilter, StudentNavigation } from "../../components";
...
export default function StudentHomePage() {
  ...
  return (
    <div className="container">
      ...
      <InternshipFilter onFilter={(filter) => refreshInternships(1, filter)} onCancel={() => {refreshInternships(1,{})}} ></InternshipFilter>
      {pagedInternships.data.map((internship) => (
        <Internship key={internship.id} internship={internship} buttonText={"Details"} hasApplicationsCount={false} redirectTo={() => {navigate('/login')}}/>
      ))}
      <Paging
